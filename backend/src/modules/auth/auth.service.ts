...
export class AuthService {
  ...
  async verifyOtp(mobile: string, otp: string): Promise<any> {
    const otpRecord: UserRecord[] = await this.databaseService.query(
    ...
    let user: UserRecord[] = await this.databaseService.query(
    ...
    await this.databaseService.execute(
      ...
      [uuidv4(), userData.id, token, refreshToken]
    );
  ...
  async refreshToken(refreshToken: string): Promise<any> {
    try {
      ...
      const user: UserRecord[] = await this.databaseService.query(
        `SELECT * FROM users WHERE id = ?`,
        [payload.sub]
      );
  ...
  async getProfile(userId: string): Promise<any> {
    ...
    const agentProfile: UserRecord[] = await this.databaseService.query(
